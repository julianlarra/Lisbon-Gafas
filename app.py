...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://lisbongafas:databricks@lisbongafas.mysql.pythonanywhere-services.com/lisbongafas$default"
...
class Producto(db.Model):  # Producto hereda de db.Model
    ...
    def __init__(self, codigo_producto, linea, modelo, color, precio, stock, imagen):
        self.codigo_producto = codigo_producto   # no hace falta el id porque lo crea sola mysql por ser auto_incremento
        self.linea = linea
        self.modelo = modelo
        self.color = color
        self.precio = precio
...
class ProductoSchema(ma.Schema):
    ...
    """
    class Meta:
        fields=('id','codigo_producto','linea','modelo','color' ,'precio' ,'stock' ,'imagen')
...
def create_producto():
    ...
    new_producto=Producto(codigo_producto,linea,modelo,color,precio,stock,imagen)  # Crea un nuevo objeto Producto con los datos proporcionados
